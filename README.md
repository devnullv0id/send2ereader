...
Images are built for `linux/amd64` and `linux/arm64`. About 105 MB to pull, 470 MB unpacked.
Every release is built on both `ghcr.io` and `code.private-home-network.de` from the same source,
each forge publishing its own build; take whichever you can reach.

| Tag               | What it is                                         |
|-------------------|----------------------------------------------------|
| `latest`          | Current release                                    |
| `2.0.7`, `2.0`    | Pin to a version or a minor series                 |
...
Extensions come from their own images, published beside the app and tagged with the app version
they shipped with. They are rebuilt only when the extension itself changes, so those versions are
sparser than the app's: `latest` is what an id installs, and a tag like `2.0.6` holds one still.
`EXTENSIONS` takes an image name as readily as an id, which is how you pin one:

```yaml
environment:
  EXTENSIONS: calibre|ghcr.io/devnullv0id/s2e-mod-kfx:2.0.6
```
...
1. Copy [`languages/de.json`](languages/de.json) to `<code>.json` — `fr.json`, `nl.json` — and translate the values. The keys are the English sentences themselves; a key you leave out simply stays English, so a half-finished file is safe to ship.
2. Put it in the `languages` folder of a checkout, or on the data volume under `/data/languages` for a Docker install (`docker cp fr.json send2ereader:/data/languages/`).
...
- `download.html`, `style.css`, `common.js` and `receive.js` run **on the ereader**, in WebKit builds from the early 2010s: no custom properties, flexbox, grid, `const`, arrow functions or `fetch`. Black on white, because e-ink turns greys into noise.
- Everything else is a phone or desktop browser. No `style=` attributes, no HTML built from strings — repeated rows clone a `<template>`.
